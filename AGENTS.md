...
- `get_financials`: primary fundamentals tool for financial statements, key ratios, metrics snapshots, and analyst estimates. Handles multi-company requests in one call.
- `get_market_data`: primary market data tool for prices, quotes, company news, and insider trades. Handles multi-asset requests in one call.
...
- Finance: `FMP_API_KEY`, `FINNHUB_API_KEY`
